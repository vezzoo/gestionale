...
class Cassa extends React.Component {
    ...
    reloadList() {
        this.images = [];
        return GET(apiCalls.productList).then(res =>
    ...
    genScontrino() {
        POST(apiCalls.getOrdNum, {
            ...
            if (res.state) {
                ...
                this.ordnum = res;
                let pagato = this.state.payed === "" ? "0.00" : parseFloat(this.state.payed).toFixed(2);
                let resto = this.state.payed === "" ? "0.00" : (-Math.floor(((this.total[0] + this.total[1] / 100) - parseFloat(this.state.payed)) * 100) / 100).toFixed(2);

                this.scontrino =
                    <Scontrino
                        path={scontrinoModel}
                        elementi={this.getBillData()}
                        onScontrinoLoaded={() => this.setState({stampaDisabled: false})}
                        kw={{
    ...
    render() {
        ...
        return (
            <div className={this.props.classes.root}>
                            ...
                            return (<ProductTile tabspace={tabSpace} dispon={e.dispon} desc={e.desc} eur={e.eur}
                                                 ...
                                                 onClick={(evt) => {
                                                     if (!e.details.display) {
                                                         if ((this.state.subtract || evt.shiftKey) && e.qta > 0) e.qta -= 1;
                                                         else if (!(this.state.subtract || evt.shiftKey)) e.qta += 1;
                    ...
                    <Grid container spacing={12}>
                        ...
                        <Grid item xs={12} md={3}>
                            ...
                            {window.ctx.get("isLogged") && <Typography
                                variant="title">Resto: {this.state.payed === "" ? 0.0 : -Math.floor(((this.total[0] + this.total[1] / 100) - parseFloat(this.state.payed)) * 100) / 100} {Currency}</Typography>}
                            <FormControlLabel
                                control={
                                    <Checkbox
                ...
                <Dialog open={this.state.isAddOpen} onClose={() => {
                    this.needSelect = false;
                    this.hasSelected = false;
                    this.setState({isAddOpen: false, selectSelected: undefined})
                }}
                    ...
                    <DialogContent>
                        <div>
                            ...
                            <Button variant="contained" color="primary" disabled={this.needSelect && !this.hasSelected}
                                    onClick={() => {
                                        ...
                                        this.state.cart.forEach(e => {
                                            if (e.desc === this.state.lastEdited.desc && (JSON.stringify(e.variant) === JSON.stringify(this.state.lastEdited.variant))) {
                                                e.qta++;
                                                fattone = false;
                                            }
                                        });

                                        if (fattone) this.state.cart.push(JSON.parse(JSON.stringify(this.state.lastEdited)));
