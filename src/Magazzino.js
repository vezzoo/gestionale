...
class Magazzino extends React.Component {
    ...
    componentDidMount() {
        ...
        }).then(() => POST(apiCalls.getGruppiCucina, {
        ...
        }).then(() => POST(apiCalls.getProducts, {
            user: window.ctx.get("username"),
            token: window.ctx.get("token")
        }).then(res => {
            if (res.state) {
    ...
    render(start) {
        ...
        return (
            <div>
                <NavBar
                    titleText={'Magazzino - ' + (this.state.current === 1 ? 'Prodotti' : (this.state.current === 2 ? 'Dettagli' : (this.state.current === 3 ? 'Gruppi' : '')))}
                                                    ...
                                                    return e.json.dialog[e1].map((e2, i1) => {
                                                        return (
                                                            <ListItem style={{margin: 0, padding: 2}}>
                                                                ...
                                                                <ListItemText>
                                                                    <TextField
                                                                        placeholder={e2.charAt(0).toUpperCase() + e2.slice(1)}
                                                                        onChange={e3 => {
                                                                ...
                                                                <Button disabled={e.json.dialog[e1].length < 2}
                                                                        onClick={() => {
