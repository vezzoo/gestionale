...
const queries = [
    ...
    },
    {
        ...
        type: "bar",
        query: (fromTime, toTime) => `select sum(foo.totale) as y, foo.usr as label from (select (m.prezzoEur + m.prezzoCents / 100) * op.qta as totale, od.user as usr from ordini_prodotti as op inner join magazzino as m on op.product = m.id inner join ordini_dettagli od on op.order = od.id) as foo group by foo.usr;\n`
    },
    {
        group: "ordini",
    ...
    {
        group: "ordini",
        name: "incassi totali per pietanza",
        type: "bar",
        query: (fromTime, toTime) => `SELECT COALESCE(SUM(qta*(SELECT \`timestamp\` > ${fromTime} AND \`timestamp\` < ${toTime} FROM ordini_dettagli WHERE id = \`order\`)), 0)  * (max(magazzino.prezzoEur) + max(magazzino.prezzoCents) / 100) as y, magazzino.descrizione as label from ordini_prodotti right join magazzino on ordini_prodotti.product = magazzino.id  GROUP BY magazzino.descrizione;\n`
    }
];
