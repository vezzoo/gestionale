...
module.exports.callback = function (proxy) {
    proxy.getConnection().query(`select 
        ...
        r.forEach(e => {
            ...
            orders[e.id].totale[1] += (e.cents * e.qta) % 100;
            orders[e.id].totale[0] += (e.eur * e.qta) + Math.floor(orders[e.id].totale[1] / 100);
        });

        proxy.send({state: true, list: orders})
    })
};